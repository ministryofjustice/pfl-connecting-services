...
In the future, we may want to persist data permanently, to allow people to come back to revisit or edit their data.
...
- We could use the GOV.UK One Login service for a full sign-in experience. In this case we would have an internal GUID
  ...
  multiple sessions, so we'd have to consider how to handle that use case if required
...
- We could collect an email, and then have the user enter their email and get sent a code/magic link to log in. We could
  include a reference number to allow them to have multiple sessions simultaneously. This might cause friction in the user
